  ...
  return `
    <div class="card-structural">
      ...
      <div class="border-2 border-dashed border-bp-light/40 rounded-sm p-8 text-center mb-6 hover:border-bp-accent transition cursor-pointer" id="upload-dropzone">
        ...
        <p class="text-sm text-neutral-600 mt-1 font-mono">PDF, DOCX, TXT SUPPORTED</p>
        <input id="file-input" type="file" multiple accept=".pdf,.docx,.txt,.md" class="hidden">
      </div>
      <div class="grid md:grid-cols-2 gap-4 mb-4">
        <div>
          <label class="text-sm text-neutral-500 block mb-1 font-mono">DOCUMENT TYPE</label>
...
async function handleFileUpload(input) {
  ...
  for (const file of files) {
    ...
    item.innerHTML = `<i class="fas fa-file-alt text-bp-accent text-xs"></i><span class="text-sm text-white flex-1 font-mono">${file.name}</span><span class="text-xs text-neutral-600 font-mono">UPLOADING...</span>`;
